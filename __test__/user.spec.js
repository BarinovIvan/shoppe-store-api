...
describe('testing user API', () => {
  it('get all users', async () => {
    const response = await supertest(app).set('Cookie', 'authorization=amigo').get('/users');
  ...
  it('get a single user', async () => {
    const response = await supertest(app).set('Cookie', 'authorization=amigo').get('/users/2');
    expect(response.status).toBe(200);
    console.log(response.body);
    expect(response.body).not.toStrictEqual({});
  ...
  it('delete a user', async () => {
    const response = await supertest(app).set('Cookie', 'authorization=amigo').delete('/users/2');
    expect(response.status).toBe(200);
    console.log(response.body);
    expect(response.body).toHaveProperty('id');
  });
